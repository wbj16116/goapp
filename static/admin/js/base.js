...
var app={
	...
	changeStatus:function(){
		$(".chStatus").click(function(){
			...
			var el=$(this)
			$.get("/"+config.adminPath+"/changeStatus",{id:id,table:table,field:field},function(response){
				if(response.success){
		...
		*/
		...
		$(".chSpanNum").click(function(){
			...
			$(input).blur(function(e){
				...
				spanEl.html(inputNum);
				//异步请求修改数量
				$.get("/"+config.adminPath+"/editNum",{id:id,table:table,field:field,num:inputNum},function(response){
					if(!response.success){
						console.log(response)
					}
				})
